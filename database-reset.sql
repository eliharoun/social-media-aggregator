...
-- This will clean up existing policies and recreate everything fresh
...
BEGIN
    ...
    FOR r IN (
        ...
        AND tablename IN ('favorite_creators', 'content', 'transcripts', 'summaries', 'user_settings', 'user_profiles', 'user_content_interactions')
    ) LOOP
...
ALTER TABLE IF EXISTS public.favorite_creators DISABLE ROW LEVEL SECURITY;
...
ALTER TABLE IF EXISTS public.user_content_interactions DISABLE ROW LEVEL SECURITY;
...
-- Create Row Level Security Policies (Fresh)
...
-- Simple transcript policies (no complex JOINs)
...
-- Simple summary policies (no complex JOINs)
CREATE POLICY "Authenticated users can view summaries" ON public.summaries
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can insert summaries" ON public.summaries
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can update summaries" ON public.summaries
  FOR UPDATE USING (auth.role() = 'authenticated');
...
DROP TRIGGER IF EXISTS handle_user_content_interactions_updated_at ON public.user_content_interactions;
